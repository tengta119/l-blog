        ...
        files.map((file) => {
            return new Promise((rev, rej) => {
                ...
                uploadFile(formData).then((res) => {
                    ...
                    console.log('访问路径：' + data.data.url)
...
// 渲染标签数据
searchTags({key: ''}).then(res => {
    console.log("获取标签数据", res.data.data)
...
const handleUpdateCoverChange = (file) => {
    ...
    uploadFile(formData).then((e) => {
        ...
        // 成功则设置表单对象中的封面链接，并提示上传成功
        updateArticleForm.cover = data.data.url
        showMessage('上传成功')
    })
}
...
const showArticleUpdateEditor = (row) => {
    ...
    getArticleDetail({id: articleId}).then((res) => {
        const data = res.data
        console.log(data)
        if (data.code == '0000') {
            // 设置表单数据
            updateArticleForm.id = data.data.id
            updateArticleForm.title = data.data.title
            updateArticleForm.cover = data.data.cover
            updateArticleForm.content = data.data.content
            updateArticleForm.categoryId = data.data.categoryId
            updateArticleForm.tags = data.data.tagIds
            updateArticleForm.summary = data.data.summary
        }
    })
}
