<template>
    ...
    <div v-if="titles && titles.length > 0" ref="tocRef"
        class="sticky top-[5.5rem] text-sm/[30px] w-full p-5 mb-3 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 max-h-[calc(100vh-7rem)] overflow-y-auto overscroll-contain toc-scroll">
        <!-- 目录标题 -->
        ...
        <div class="border-l-2 border-gray-200">
            <ul>
                <!-- 二级标题 -->
                <li v-for="(h2, index) in titles" :key="index" class="pl-5 toc-item" :data-index="h2.index" :class="[h2.index == activeHeadingIndex ? 'active text-sky-600 border-l-2 border-sky-600 font-bold' : 'text-gray-500 font-normal']">
                    <span @click="scrollToView(h2.offsetTop)" class="cursor-pointer">{{ h2.text }}</span>
                    <!-- 三级标题 -->
                    <ul v-if="h2.children && h2.children.length > 0">
                        <li v-for="(h3, index2) in h2.children" :key="index2" class="pl-5 toc-item" :data-index="h3.index" :class="[h3.index == activeHeadingIndex ? 'active text-sky-600 border-l-2 border-sky-600 font-bold' : 'text-gray-500 font-normal']">
                            <span @click="scrollToView(h3.offsetTop)" class="cursor-pointer">{{ h3.text }}</span>
                        </li>
                    </ul>
                </li>
            </ul>
...
watch(activeHeadingIndex, async (idx) => {
    await nextTick();
    const c = tocRef.value;
    if (!c) return;
    const activeEl = c.querySelector('.toc-item.active');
...
.toc-scroll:hover::-webkit-scrollbar {
  width: 8px;
  height: 8px;
}
